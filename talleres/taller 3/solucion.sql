...
/*
    NOTA
    las siguientes consultas de abajo solo funcionan cuando se pone la columna nota_promedio como la primer cluster clomun
*/

    -- La nota mas alta del anyio 2019
SELECT numero_anyo_asignatura, NIE_alumno, nombre_alumno,nota_promedio
FROM notas_test
WHERE numero_anyo_asignatura = 2019
...
;
    ...
    -- La nota mas alta del anyio 2018
SELECT numero_anyo_asignatura, NIE_alumno, nombre_alumno,nota_promedio
FROM notas_test
WHERE numero_anyo_asignatura = 2018
ORDER BY nota_promedio DESC
...
;
    ...
    -- Comprobacion manual
SELECT * FROM notas_test WHERE numero_anyo_asignatura = 2018 ORDER BY nota_promedio DESC;
SELECT * FROM notas_test WHERE numero_anyo_asignatura = 2019 ORDER BY nota_promedio DESC;


/*
    TABLA CON EL CLUSTER COLUMN ORDER ALTERADO PARA QUE FUNCIONEN LAS CONSULTAS ANTERIORES + INSERTS A ESTA TABLA
*/


CREATE TABLE notas_test(
    NIE_alumno varchar,
    denominacion_nivel_grado_asignatura varchar,
    numero_grado_asignatura tinyint,
    numero_anyo_asignatura int,
    ...
    nombre_alumno varchar,   
    nota_promedio decimal,
    PRIMARY KEY (numero_anyo_asignatura, nota_promedio, denominacion_nivel_grado_asignatura, numero_grado_asignatura, nombre_asignatura, NIE_alumno)
)WITH CLUSTERING ORDER BY (nota_promedio DESC);


    -- 1
INSERT INTO notas_test (NIE_alumno,denominacion_nivel_grado_asignatura,numero_grado_asignatura,numero_anyo_asignatura,nombre_asignatura,nombre_alumno,nota_promedio)
VALUES ('0000001-1','basica',6,2018,'Artistica','Poncio Pilato',8);
